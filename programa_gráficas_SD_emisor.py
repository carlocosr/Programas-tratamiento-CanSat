...
COLUMNAS_ESPERADAS = [
    ...
    "Latitud", "Longitud", "AccZ", "AccX", "AccY",
    "GyroX", "GyroY", "GyroZ", "Fecha", "Hora"
]

archivo = "datos_emisor.csv" # Para el emisor
...
def generar_graficas_interactivas(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        # Acelerómetro X
        trace_accx = px.line(df, x='Paquete', y='AccX').data[0]
        fig3.add_trace(trace_accx, row=1, col=1)

        # Acelerómetro Y
        trace_accy = px.line(df, x='Paquete', y='AccY').data[0]
...
def main():
    ...
    try:
        ...
        for nombre, figura in figuras.items():
            ruta_html = f"graficos/{nombre}_emisor.html"
